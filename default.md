...
- When the user says "yolo", make decisions autonomously for the specific task at hand — not for the rest of the session. Once that task is complete, return to normal (ask before acting on new tasks). For non-trivial decisions within the yolo scope: (1) propose an answer with justification, (2) pressure-test it against the strongest alternative, (3) commit and move on. Proceed without waiting. Document non-obvious decisions with reasoning — in the current document if writing one, otherwise in chat
